#!/usr/bin/env python3
"""
PPO Training v6 — 60 features (v5 env + order book + trade flow).

New features (15 total):
  Order Book (15): imbalance, spread_bps, depth_5/20 bid/ask, depth_imbalance_5/20,
                    wall_bid, wall_ask, slope_bid, slope_ask, spread, depth_10 bid/ask
  Trade Flow (8): buy_ratio, flow_imbalance, large_trades, avg_size, size_variance,
                   aggression, buy_volume, sell_volume

Based on v12 (small positions, max hold 20).
"""
...
from environment_v4 import PolymarketEnvV5 as PolymarketEnv, MIN_HOLD_STEPS
...
def make_env(asset, data_path, seed=42):
    def _init():
        return PolymarketEnv(
            ...
            position_size_pct=0.02,  # small positions (from v12)
            taker_fee=0.025,
...
def train(
    asset="btc",
    data_path="/home/antonov5/.openclaw/workspace/rl_polymarket/data/expanded_snapshots_v2.jsonl",
    total_steps=500_000,
    seed=42,
    save_dir="/home/antonov5/.openclaw/workspace/rl_polymarket/models",
):
    print("=" * 60)
    print(f"  PPO Training v6 — {asset.upper()} — 60 features")
    print(f"  Steps: {total_steps:,}")
    print(f"  Data: {data_path}")
    print(f"  Position size: 2% | Max hold: 20 steps")
    print("=" * 60)

    # Check data exists
    if not os.path.exists(data_path):
        print(f"[ERROR] Data file not found: {data_path}")
        print("Run expand_with_orderbook.py first to generate v2 data")
        return
    ...
    # PPO model — same hyperparams as v12
    model = PPO(
        "MlpPolicy",
        env,
        learning_rate=3e-4,
    ...
    # Save final model
    final_path = os.path.join(save_dir, f"ppo_v6_{asset}_steps{total_steps}")
    ...
    # Save stats
    stats_path = os.path.join(save_dir, f"ppo_v6_{asset}_eval.json")
...
if __name__ == "__main__":
    ...
    data_path = args.data or f"/home/antonov5/.openclaw/workspace/rl_polymarket/data/expanded_snapshots_v2.jsonl"
